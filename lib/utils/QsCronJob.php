...
/**
 * QsCronJob represents single line of cron configuration file (crontab) - single cron job.
 ...
 * @package qs.utils
 */
class QsCronJob extends CModel {
	/**
	...
	public function getLine($runValidation = true) {
		if ($runValidation && !$this->validate()) {
			throw new CException($this->getErrorSummary());
		}
	...
	public function rules() {
		return array(
			array('min, hour, day, month, weekDay, command', 'required'),
			array('min, hour, year', 'match', 'pattern' => '/^[0-9\*\/\,\-]+$/s'),
			array('month', 'match', 'pattern' => '/^[A-Z0-9\*\/\,\-]+$/s'),
			array('day', 'match', 'pattern' => '/^[0-9\*\/\,\-\?LW]+$/s'),
			array('weekDay', 'match', 'pattern' => '/^[A-Z0-6\*\/\,\-L#]+$/s'),
		);
	}
	 ...
	 * @throws CException on invalid line format.
	 */
	protected function parseLine($line) {
		...
		if (count($parts) < $partsCount) {
			throw new CException('Cron job line "' . $line . '" invalid.');
		}
